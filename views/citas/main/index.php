<div class="container menu-catalog">
    ...
    <div class="card form-card shadow-sm border-0 mt-4">
                    ...
                    <tbody>
                        <?php foreach ($this->citas as $cita): ?>
                            <tr>
                                ...
                                <td><i class="bi bi-clock text-muted me-1"></i>
                                ...
                                <td class="text-end pe-4">
                                    <div class="d-flex justify-content-end gap-2 align-items-center">
                                        ...
                                        <?php if ($_SESSION['role_id'] == 1): ?>
                                            <a href="<?= URL ?>citas/edit/<?= $cita->id ?>"
                                                class="btn btn-sm btn-outline-dark">Gestionar</a>
                                        <?php else: ?>
                                        ...
                                        <?php if ($cita->estado === 'Finalizada'): ?>
                                            <a href="<?= URL ?>citas/factura/<?= $cita->id ?>" target="_blank"
                                                class="btn btn-sm btn-danger" title="Descargar Factura">
        ...
        <?php else: ?>
            <div class="empty-state-container text-center py-5 shadow-sm rounded-4 bg-white border">
                <div class="empty-state-icon mb-4">
                    <i class="bi bi-calendar2-x text-muted opacity-25" style="font-size: 5rem;"></i>
                </div>
                <h3 class="fw-bold text-dark">Aún no hay reservas</h3>
                <p class="text-muted mb-4 mx-auto" style="max-width: 400px;">
                    Parece que todavía no has solicitado ninguna experiencia gastronómica.
                    ¡Rafael está deseando cocinar para ti!
                </p>
                <a href="<?= URL ?>menu" class="btn btn-primary btn-lg px-5 shadow-sm">
                    Explorar la Carta
                </a>
            </div>
        <?php endif; ?>
